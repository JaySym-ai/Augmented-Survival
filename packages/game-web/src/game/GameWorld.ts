...
export class GameWorld {
  ...
  placeDebugCompletedBuilding(type: BuildingType, position: Vector3): EntityId | null {
    if (!import.meta.env.DEV) {
      console.warn('[Augmented Survival] Debug building placement is only available in development builds.');
      return null;
    }
    ...
    const placedPosition = {
      ...position,
      y: getMaxHeightForFootprint(this.terrainMesh, position.x, position.z, def.size.width, def.size.depth),
    };

    const entityId = this.buildingPlacement.placeBuilding(this.world, type, placedPosition, {
      cost: def.cost,
      workerSlots: def.workerSlots,
      storageCapacity: def.storageCapacity,
      buildTime: def.buildTime,
    ...
    const mesh = this.createBuildingVisual(type, placedPosition);
    mesh.position.set(placedPosition.x, placedPosition.y, placedPosition.z);
    mesh.castShadow = true;
    ...
    if (type === BuildingType.TownCenter) {
      this.raiseTownCenterCampfireTerrain(placedPosition);
    }
    this.terrainMesh.refreshGeometry();
